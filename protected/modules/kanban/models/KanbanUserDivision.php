...
class KanbanUserDivision extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('division_id, publish, parent, name, desc, management, tested, verified, creation_date', 'safe', 'on'=>'search'),
		);
	}
	...
	protected function afterConstruct() {
		if(count($this->defaultColumns) == 0) {
			...
			*/
			...
			$this->defaultColumns[] = array(
				'name' => 'parent',
				'value' => '$data->parent != 0 ? KanbanUserDivision::model()->findByPk($data->parent)->name : "-"',
			);
			$this->defaultColumns[] = array(
				'name' => 'management',
	...
	public static function getDivision($publish=null) {
		...
		if($model != null) {
			foreach($model as $key => $val) {
				$items[$val->division_id] = $val->name;
			}
			return $items;
		} else {
			return false;
